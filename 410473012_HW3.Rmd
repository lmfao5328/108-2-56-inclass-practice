...
pick_vio = !is.na(violatedArticles)
violatedArticles = violatedArticles[pick_vio]
...
```{r ans13}
max_num = max(table_countsByArticle) #找最大
pick_max = table_countsByArticle == max_num #挑符合最大的項目
mostViolatedArticle = table_countsByArticle[pick_max] #轉存
...
#試縮成一行
mostViolatedArticle_content = names(
  table_lawcontent[
    table_lawcontent == max(table_lawcontent)
  ]
)
...
```{r ans15}
months_table = table(str_sub(badEmployers$dt,6,7))
names(
  months_table[
    months_table == max(months_table)
  ]
...
```{r ans22}
pick_namelist = str_detect(students10$志願, "北大")
ntpu_namelist = students10$姓名[pick_namelist]
...
ntpu_eco = students10$姓名[pick_eco]
ntpu_eco
```
...
```{r ans24}
namelist[namelist %in% ntpu_eco] -> eco_new
congrats = paste0("恭喜 ", eco_new, " 同學錄取國立臺北大學經濟系")
congrats
# congrats
```
