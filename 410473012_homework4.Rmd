...
```{r data1}
twoNames <- c("有人叫他段兄","，但也有有人叫他段兄弟，最常....")
...
```{r ans21}
temp = semiGodsSemiDevils$data[[1]]
context_picked = temp$text[
  temp$section == "P2"
]
context_picked
```
...
```{r ans22}
context_picked_split =  as.list(str_split(context_picked, "\\s{2}"))
context_picked_split = as.list(context_picked_split[1])
...
```{r ans23}
mention_duan = str_extract_all(context_picked_split, "(段|姓段)(譽|兄|兄弟|大哥|相公|君|的小子|世兄)(?!弟)")
mention_duan
```
